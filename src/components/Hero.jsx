          ...
          <div className="w-full grid lg:grid-cols-4">
            <div className="w-full md:col-span-3  flex items-center">
              {HeroData.map((d, index) => (
                <span key={index}>
                  <p className="text-primary-ylogo font-black text-7xl md:text-8xl md:overflow-hidden pb-10 pt-8 md:pt-16 overflow-hidden">
                    {d.name}
                  </p>
          ...
          <div className="block md:w-3/5">
            <div>
              {ServiceData.map((d, index) => (
                <p
                  key={index}
                  className="text-xl text-white leading-loose overflow-hidden"
                >
                  {d.service}
                </p>
              ))}
